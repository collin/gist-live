...
EditAreaLoader.prototype ={
    ...
    console.log('lang/syntax loaded')
		...
		console.log("build html, or something")
	...
	*/	
	...
	set_base_url : function(){
		//this.baseURL="";
		if (!this.baseURL) {
      this.baseURL = "http://localhost:4567/edit_area"
// 			var elements = document.getElementsByTagName('script');
// 	
// 			for (var i=0; i<elements.length; i++) {
// 				if (elements[i].src && elements[i].src.match(/embedded[^\\\/]*$/i) ) {
// 					var src = elements[i].src;
// 					src = src.substring(0, src.lastIndexOf('/'));
// 					this.baseURL = src + "edit_area";
...
// 			}
	...
	get_template : function(){
		this.template = '<?xml version="1.0" encoding="UTF-8"?>\r<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">\r<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" >\r<head>\r\t<title>EditArea</title>\r\t<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />\r\t[__CSSRULES__]\r\t[__JSCODE__]\r</head>\r<body>\r\t<div id="editor">\r\t\t<div class="area_toolbar" id="toolbar_1">[__TOOLBAR__]</div>\r\t\t<div class="area_toolbar" id="tab_browsing_area"><ul id="tab_browsing_list" class="menu"> <li> </li> </ul></div>\r\t\t<div id="result">\r\t\t\t<div id="no_file_selected"></div>\r\t\t\t<div id="container">\r\t\t\t\t<div id="cursor_pos" class="edit_area_cursor">&nbsp;</div>\r\t\t\t\t<div id="end_bracket" class="edit_area_cursor">&nbsp;</div>\r\t\t\t\t<div id="selection_field"></div>\r\t\t\t\t<div id="line_number" selec="none"></div>\r\t\t\t\t<div id="content_highlight"></div>\r\t\t\t\t<div id="test_font_size"></div>\r\t\t\t\t<textarea id="textarea" wrap="off" onchange="editArea.execCommand("onchange");" onfocus="javascript:editArea.textareaFocused=true;" onblur="javascript:editArea.textareaFocused=false;">\r\t\t\t\t</textarea>\r\t\t\t\t\r\t\t\t</div>\r\t\t</div>\r\t\t<div class="area_toolbar" id="toolbar_2">\r\t\t\t<table class="statusbar" cellspacing="0" cellpadding="0">\r\t\t\t\t<tr>\r\t\t\t\t\t<td class="total" selec="none">{$position}:</td>\r\t\t\t\t\t<td class="infos" selec="none">\r\t\t\t\t\t\t{$line_abbr} <span  id="linePos">0</span>, {$char_abbr} <span id="currPos">0</span>\r\t\t\t\t\t</td>\r\t\t\t\t\t<td class="total" selec="none">{$total}:</td>\r\t\t\t\t\t<td class="infos" selec="none">\r\t\t\t\t\t\t{$line_abbr} <span id="nbLine">0</span>, {$char_abbr} <span id="nbChar">0</span>\r\t\t\t\t\t</td>\r\t\t\t\t\t<td class="resize">\r\t\t\t\t\t\t<span id="resize_area"><img src="[__BASEURL__]images/statusbar_resize.gif" alt="resize" selec="none"></span>\r\t\t\t\t\t</td>\r\t\t\t\t</tr>\r\t\t\t</table>\r\t\t</div>\r\t</div>\r\t<div id="processing">\r\t\t<div id="processing_text">\r\t\t\t{$processing}\r\t\t</div>\r\t</div>\r\r\t<div id="area_search_replace" class="editarea_popup">\r\t\t<table cellspacing="2" cellpadding="0" style="width: 100%">\r\t\t\t<tr>\r\t\t\t\t<td selec="none">{$search}</td>\r\t\t\t\t<td><input type="text" id="area_search" /></td>\r\t\t\t\t<td id="close_area_search_replace">\r\t\t\t\t\t<a onclick="Javascript:editArea.execCommand("hidden_search")"><img selec="none" src="[__BASEURL__]images/close.gif" alt="{$close_popup}" title="{$close_popup}" /></a><br />\r\t\t\t</tr><tr>\r\t\t\t\t<td selec="none">{$replace}</td>\r\t\t\t\t<td><input type="text" id="area_replace" /></td>\r\t\t\t\t<td><img id="move_area_search_replace" onmousedown="return parent.start_move_element(event,"area_search_replace", parent.frames["frame_"+editArea.id]);"  src="[__BASEURL__]images/move.gif" alt="{$move_popup}" title="{$move_popup}" /></td>\r\t\t\t</tr>\r\t\t</table>\r\t\t<div class="button">\r\t\t\t<input type="checkbox" id="area_search_match_case" /><label for="area_search_match_case" selec="none">{$match_case}</label>\r\t\t\t<input type="checkbox" id="area_search_reg_exp" /><label for="area_search_reg_exp" selec="none">{$reg_exp}</label>\r\t\t\t<br />\r\t\t\t<a onclick="Javascript:editArea.execCommand("area_search")" selec="none">{$find_next}</a>\r\t\t\t<a onclick="Javascript:editArea.execCommand("area_replace")" selec="none">{$replace}</a>\r\t\t\t<a onclick="Javascript:editArea.execCommand("area_replace_all")" selec="none">{$replace_all}</a><br />\r\t\t</div>\r\t\t<div id="area_search_msg" selec="none"></div>\r\t</div>\r\t<div id="edit_area_help" class="editarea_popup">\r\t\t<div class="close_popup">\r\t\t\t<a onclick="Javascript:editArea.execCommand("close_all_inline_popup")"><img src="[__BASEURL__]images/close.gif" alt="{$close_popup}" title="{$close_popup}" /></a>\r\t\t</div>\r\t\t<div><h2>Editarea [__EA_VERSION__]</h2><br />\r\t\t\t<h3>{$shortcuts}:</h3>\r\t\t\t\t{$tab}: {$add_tab}<br />\r\t\t\t\t{$shift}+{$tab}: {$remove_tab}<br />\r\t\t\t\t{$ctrl}+f: {$search_command}<br />\r\t\t\t\t{$ctrl}+r: {$replace_command}<br />\r\t\t\t\t{$ctrl}+h: {$highlight}<br />\r\t\t\t\t{$ctrl}+g: {$go_to_line}<br />\r\t\t\t\t{$ctrl}+z: {$undo}<br />\r\t\t\t\t{$ctrl}+y: {$redo}<br />\r\t\t\t\t{$ctrl}+e: {$help}<br />\r\t\t\t\t{$ctrl}+q, {$esc}: {$close_popup}<br />\r\t\t\t\t{$accesskey} E: {$toggle}<br />\r\t\t\t<br />\r\t\t\t<em>{$about_notice}</em>\r\t\t\t<br /><div class="copyright">&copy; Christophe Dolivet - 2007</div>\r\t\t</div>\r\t</div>\r\t</div>\r</body>\r</html>\r';
	},
